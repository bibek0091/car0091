...
class DashboardUI:
    ...
    def _setup_layout(self):
        ...
        # Additional HUD items
        self.lbl_parking = tk.Label(status, text="PARK: NONE", bg=THEME["panel"], fg="white", font=("Courier", 10))
        self.lbl_parking.pack(side=tk.RIGHT, padx=8)
        self.lbl_imu = tk.Label(status, text="IMU DISCONNECTED | YAW: 0.0°", bg=THEME["panel"], fg="#ccc", font=("Courier", 10))
        self.lbl_imu.pack(side=tk.RIGHT, padx=8)
        self.lbl_model = tk.Label(status, text="MODEL: best.onnx", bg=THEME["panel"], fg="#9acd32", font=("Courier", 10))
        self.lbl_model.pack(side=tk.RIGHT, padx=8)
        ...
        # Lane-loss overlay on camera (hidden by default)
        self.lane_loss_overlay = tk.Label(cam_frm, text="LANE LOST", bg="#660000", fg="#ffdddd", font=("Helvetica", 18, "bold"))
        self.lane_loss_overlay.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
        self.lane_loss_overlay.lower(self.cam_label)
        self.lane_loss_overlay.place_forget()
        ...
        self.btn_debug = tk.Checkbutton(sys_frm, text="DEBUG", variable=self.debug_enabled, bg="#444", fg="white", selectcolor="#333", font=("Helvetica", 9, "bold"))
        self.btn_debug.grid(row=3, column=0, columnspan=2, sticky="ew", padx=3, pady=3)
    ...
    def update_hud(self, speed=None, steer=None, mode=None, parking_state=None, imu_yaw=None, imu_connected=None, lane_status=None, ai_status=None):
        """Update compact HUD elements displayed in the top bar."""
        try:
            if speed is not None or steer is not None or mode is not None:
                mode_str = mode if mode else ""
                spd = int(speed) if speed is not None else 0
                str_deg = f"{steer:.1f}" if steer is not None else "0.0"
                self.lbl_telemetry.config(text=f"SPD: {spd} | STR: {str_deg}° | [{mode_str}]")
            if parking_state is not None:
                self.lbl_parking.config(text=f"PARK: {parking_state}")
            if imu_yaw is not None:
                if imu_connected:
                    self.lbl_imu.config(text=f"IMU CONNECTED | YAW: {imu_yaw:.1f}°", fg="#9acd32")
                else:
                    self.lbl_imu.config(text=f"IMU DISCONNECTED | YAW: {imu_yaw:.1f}°", fg="#ff3333")
            if ai_status is not None:
                self.lbl_ai.config(text=f"AI: {ai_status}")
            # lane_status controls overlay
            if lane_status == "LOST":
                self.lane_loss_overlay.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
            else:
                self.lane_loss_overlay.place_forget()
    ...
    def toggle_debug(self, enabled: bool):
        self.debug_enabled.set(bool(enabled))
        if self.debug_enabled.get():
            self.log_event("Debug mode enabled", "INFO")
        else:
            self.log_event("Debug mode disabled", "INFO")
