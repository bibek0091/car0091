...
class ThreadedYOLODetector:
    def __init__(self, model_path=None):
        ...
        if self.model_path_used:
            print(f"[YOLO] Loading model from {self.model_path_used}...")
            self.model = YOLO(self.model_path_used, task='detect')
            self.yolo_ok = True
        else:
            self.model = None
            self.yolo_ok = False
            self.model_path_used = None
            print("[YOLO] Warning: Model file not found. Detections disabled.")
        ...
        while self.running:
            try:
                ...
                if self.model is None:
                    continue
                results = self.model.predict(source=frame, conf=0.15, iou=0.45, verbose=False)
                detections = []
...
class ParkingStateMachine:
    ...
    def update(self, dets, frame, now, imu_yaw=None):
        """
        ...
        if self.state == "ENTER":
            # Use IMU yaw to help maintain straight entry
            steer = self.ENTER_STEER
            if imu_yaw is not None:
                # If yaw deviates from initial, correct
                yaw_correction = -imu_yaw * 2.0  # Tune gain as needed
                steer += yaw_correction
            if now - self._ts > self.ENTER_DURATION:
                self.state = "WAIT"
                self._ts   = now
            return "ENTER", 0.20, steer
        ...
        if self.state == "EXIT":
            # Use IMU yaw to help maintain straight exit
            steer = self.EXIT_STEER
            if imu_yaw is not None:
                yaw_correction = -imu_yaw * 2.0
                steer += yaw_correction
            if now - self._ts > self.EXIT_DURATION:
                self.state = "DONE"
                self._ts   = now
            return "EXIT", 0.28, steer
    ...
    def process(self, frame, line_type="UNKNOWN"):
        ...
        for d in dets:
            ...
            # ── Roundabout ────────────────────────────────────────────────────
                ...
                yield_needed = False
                if approx_m < 3.0: # Close to roundabout entry
                    # Look for cars in the left half of the frame (already circulating)
                    for other_d in dets:
                        o_lbl = other_d["label"].lower()
                        if o_lbl in ("car", "truck", "bus", "motorcycle"):
                            ox1, oy1, ox2, oy2 = other_d["bbox"]
                            # If a car is in the left side and lower half (close)
                            if ox1 < fw * 0.5 and oy2 > fh * 0.4:
                                yield_needed = True
                                break
                                
                if yield_needed:
                    commit(1, "SYS_STOP", "YIELDING AT ROUNDABOUT")
                else:
