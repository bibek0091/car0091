...
class ParkingSequenceFSM:
    """
    Closed-loop IMU-based parallel/angled parking sequence:
    ...
    ENTER_STEER      = 25.0    # sharp right steer into 90-degree spot
    ENTER_ANGLE_DEG  = 90.0    # 90 degrees for perpendicular parking
    WAIT_DURATION    = 3.0     # mandatory stop in spot
    EXIT_STEER       = -25.0   # sharp left steer to pull out
    FALLBACK_TIMEOUT = 5.0     # fallback timer in case IMU drifts
    ...
    def update(self, now: float, base_speed: float, spot_clear: bool = True, current_yaw: float = 0.0):
        """
        ...
        if self.state == "SEEK":
            if spot_clear or elapsed > self.SEEK_TIMEOUT:
                ...
                log.info("PARKING: entering spot. Target Yaw Diff: %.1f deg", self.ENTER_ANGLE_DEG)
            return self.SLOW_SPEED, 0.0, "SEEK"

        if self.state == "ENTER":
            # Track absolute difference in heading
            yaw_diff = current_yaw - self.initial_yaw
            yaw_diff = (yaw_diff + 180) % 360 - 180
            
            if abs(yaw_diff) >= self.ENTER_ANGLE_DEG or elapsed > self.FALLBACK_TIMEOUT:
                self.state = "WAIT"
                self._ts   = now
        ...
        if self.state == "EXIT":
            # Steer left until we return to original track heading
            yaw_diff = current_yaw - self.initial_yaw
            yaw_diff = (yaw_diff + 180) % 360 - 180
            
            if abs(yaw_diff) < 5.0 or elapsed > self.FALLBACK_TIMEOUT:
                self.state = "DONE"
...
class BehaviorController:
    ...
    def compute(self,
                ...
                current_yaw: float = 0.0) -> BehaviorOutput:
        """
        ...
        legal_out = self._check_legal(t_res, base_steer, base_speed=zone_speed)
        if legal_out:
            return legal_out

        # ── Priority 3: MISSION (parking, roundabout, highway) ──────────────
        mission_out = self._check_mission(t_res, perc_res, now,
                                          zone_speed, base_steer, current_yaw)
        if mission_out:
            return mission_out
    ...
    def _check_legal(self, t_res, base_steer: float,
                     base_speed: float = 22.0) -> Optional[BehaviorOutput]:
        """
        ...
        # Check active_labels from YOLO for bus-related classes
        active_labels = getattr(t_res, 'active_labels', [])
        if active_labels and "BUS" in " ".join(active_labels).upper():
    ...
    def _check_mission(self, t_res, perc_res, now: float,
                       base_speed: float,
                       base_steer: float,
                       current_yaw: float) -> Optional[BehaviorOutput]:
        """
        ...
        if self.parking_fsm.active:
            ...
            speed_mult, steer_bias, park_label = self.parking_fsm.update(
                now, base_speed, spot_clear=spot_clear, current_yaw=current_yaw
            )
            if park_label == "DONE":
